...
from utils.ave_segment_metric import (
    compute_accuracy_ave_segment,
    compute_accuracy_ovave_fg_only,
)
from utils.avel_metrics import AVELMetricsAccumulator
from dataset.ovavesup import AVEDatasetV2

# ================== 常量定义 ==================
SEED = 43
OVAVE_NUM_CLASSES = 67  # OVAVE 前景类数
OVAVE_BG_CLASS = 67     # 全标签 (10,68) 中背景类 index
...
def compute_batch_losses(is_event_scores, event_scores, kl_loss,
                         vis_is_event_scores, vis_event_scores,
                         audio_is_event_scores, audio_event_scores,
                         gnn_total_loss, labels, criterion, criterion_event, bs):
    ...
    if valid_event_mask.sum() > 0:
        ...
        audio_loss_event_class = criterion_event(audio_event_scores[valid_event_mask], labels_event[valid_event_mask])
    else:
    ...
    acc_ave = compute_accuracy_ave_segment(is_event_scores, event_scores, labels, OVAVE_BG_CLASS)
    acc_fg = compute_accuracy_ovave_fg_only(is_event_scores, event_scores, labels_foreground)

    return {
        'loss_with_gnn': loss_with_gnn,
        'loss_without_gnn': loss_without_gnn,
        'kl_loss': kl_loss,
        'gnn_total_loss': gnn_total_loss,
        'acc_ave': acc_ave,
        'acc_fg': acc_fg,
    }
...
def train_epoch(model, train_dataloader, criterion, criterion_event, optimizer, epoch, args, logger):
    ...
    gnn_loss_meter = AverageMeter()
    train_acc_ave = AverageMeter()
    train_acc_fg = AverageMeter()
    ...
    for n_iter, batch_data in enumerate(train_dataloader):
        ...
        if n_iter == 0 and epoch == 0:
            ...
            logger.info("[Metric] 主指标 Acc_AVE = AVE segment 口径 | Acc_fg = OVAVE 旧口径对照\n")
        ...
        loss_dict = compute_batch_losses(
            is_event_scores, event_scores, kl_loss,
            vis_is_event_scores, vis_event_scores,
            audio_is_event_scores, audio_event_scores,
            gnn_total_loss, labels, criterion, criterion_event, bs,
        )

        loss_dict['loss_with_gnn'].backward()
        ...
        train_acc_ave.update(loss_dict['acc_ave'].item(), bs * 10)
        train_acc_fg.update(loss_dict['acc_fg'].item(), bs * 10)
        total_losses_with_gnn.update(loss_dict['loss_with_gnn'].item(), bs)
        total_losses_without_gnn.update(loss_dict['loss_without_gnn'].item(), bs)
        kl_loss_meter.update(loss_dict['kl_loss'].item(), bs)
        gnn_loss_meter.update(loss_dict['gnn_total_loss'].item(), bs)
        ...
        if n_iter % args.print_freq == 0:
            logger.info(
                ...
                f'TotalLoss {total_losses_with_gnn.val:.4f} ({total_losses_with_gnn.avg:.4f})\t'
                f'NoGNN {total_losses_without_gnn.val:.4f} ({total_losses_without_gnn.avg:.4f})\t'
                f'KL {kl_loss_meter.val:.4f}\tGNN {gnn_loss_meter.val:.4f}\t'
                f'Acc_AVE {train_acc_ave.val:.2f}% ({train_acc_ave.avg:.2f}%)\t'
                f'Acc_fg {train_acc_fg.val:.2f}% ({train_acc_fg.avg:.2f}%)'
            )

    logger.info(
        f'Train Epoch {epoch} Summary | TotalLoss {total_losses_with_gnn.avg:.4f} | '
        f'NoGNN {total_losses_without_gnn.avg:.4f} | KL {kl_loss_meter.avg:.4f} | '
        f'GNN {gnn_loss_meter.avg:.4f} | Acc_AVE {train_acc_ave.avg:.4f}% | Acc_fg {train_acc_fg.avg:.4f}%'
    )
...
def validate_epoch(model, test_dataloader, criterion, criterion_event, epoch, args, logger, eval_only=False):
    ...
    model.eval()
    model.double()
    avel_metrics = AVELMetricsAccumulator(OVAVE_BG_CLASS + 1)
    ...
    for n_iter, batch_data in enumerate(test_dataloader):
        ...
        accuracy_ave.update(loss_dict['acc_ave'].item(), bs * 10)
        accuracy_fg.update(loss_dict['acc_fg'].item(), bs * 10)
        avel_metrics.update(is_event_scores, event_scores, labels, OVAVE_BG_CLASS)
        total_losses_with_gnn.update(loss_dict['loss_with_gnn'].item(), bs)
        total_losses_without_gnn.update(loss_dict['loss_without_gnn'].item(), bs)
        kl_loss_meter.update(loss_dict['kl_loss'].item(), bs)
        gnn_loss_meter.update(loss_dict['gnn_total_loss'].item(), bs)
        ...
        if n_iter % args.print_freq == 0:
            logger.info(
                ...
                f'NoGNN {total_losses_without_gnn.val:.4f} ({total_losses_without_gnn.avg:.4f})\t'
                f'KL {kl_loss_meter.val:.4f}\tGNN {gnn_loss_meter.val:.4f}\t'
                f'Acc_AVE {accuracy_ave.val:.2f}% ({accuracy_ave.avg:.2f}%)\t'
                f'Acc_fg {accuracy_fg.val:.2f}% ({accuracy_fg.avg:.2f}%)'
            )

    metric_results = avel_metrics.compute()
    logger.info(
        f'Test Epoch {epoch} Summary | TotalLoss {total_losses_with_gnn.avg:.4f} | '
        f'NoGNN {total_losses_without_gnn.avg:.4f} | KL {kl_loss_meter.avg:.4f} | '
        f'GNN {gnn_loss_meter.avg:.4f} | Acc_fg {accuracy_fg.avg:.4f}% | '
        f"{avel_metrics.format_summary()}"
    )
    if not eval_only and writer is not None:
        writer.add_scalar('Val_epoch/Seg_F1', metric_results['seg_f1'], epoch)
        writer.add_scalar('Val_epoch/Eve_F1', metric_results['eve_f1'], epoch)
        writer.add_scalar('Val_epoch/Avg', metric_results['avg'], epoch)
    return metric_results['acc']
...
def main():
    ...
    for epoch in range(args.n_epoch):
        ...
        if ((epoch + 1) % args.eval_freq == 0) or (epoch == args.n_epoch - 1):
            ...
            if acc > best_accuracy:
                ...
                logger.info(
                    f"更新最佳模型：Acc_AVE {best_accuracy:.2f}% (Epoch {best_accuracy_epoch})"
                )
    ...
    # 15. 打印最终结果
    logger.info("===== 训练完成 =====")
    logger.info(f"最佳验证 Acc_AVE (segment)：{best_accuracy:.2f}% (Epoch {best_accuracy_epoch})")
